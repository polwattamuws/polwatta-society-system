  ...
  const createMemberTransaction = db.transaction((data) => {
    ...
    for (const pos of data.positions) {
      insertPosition.run({
        memberId,
        title: pos.title,
        yearAppointed: pos.yearAppointed,
        yearResigned: pos.yearResigned || null
      });
    }

    for (const ben of data.pastBenefits) {
      insertPastBenefit.run({
        memberId,
        deceasedName: ben.deceasedName,
        relationship: ben.relationship,
        yearDeath: ben.yearDeath,
        amount: ben.amount
      });
    }

    for (const ben of data.otherBenefits) {
      insertOtherBenefit.run({
        memberId,
